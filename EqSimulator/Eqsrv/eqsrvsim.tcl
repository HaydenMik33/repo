...
an_proc setAck {setAck streamFunction=S1F13|S2F15|S2F41|S5F3|S2F23|S2F33|S2F35|S2F37|S1F15|S1F17|S2F21|S2F27|S2F31|S2F43|S2F49|S3F17|S7F1|S7F3|S7F23|S10F3|S10F5|S10F9|S14F9|S16F11|S16F15 {good, bad}} {Tell Simulator to return good or bad ack to the given stream and function} {{0 Success}} {
	set procName [lindex [info level 0] 0]
	set logName "[dv_get >name]_log"
	an_lg $logName "$procName: starting"
...
an_proc S1F13_prereply {} {} {} {
	...
	putAckInNcparms [lindex [info level 0] 0] "COMMACK"
...
proc getValAndType {svid} {
	...
	if {$sv == ""} {
		set sv [dv_get >simdata>dvid>$svid]
	}
	if {$sv == ""} {
...
an_proc S2F23_postreply {} {} {} {
	...
	set ack [dv_get >simdata>S2F23]
       	if {$ack == "01" || [dv_get >handlers>S2F23>noreply] != ""} {
		an_lg $logName "$procName: not starting or stopping trace because bad ack set or timeoutAll set"
	} else {
...
proc putAckInNcparms {scriptName variableName} {
	...
	set ack [dv_get >simdata>$streamFunction]
		...
		an_lg $logName "$procName: setting >ncparms>$variableName $ack"
	...
	if {$currentControlState == "Off-Line"} {
		...
		if {[dv_get >ncparms>HCACK] == "00"} {
			...
			if {$rcmd == "LOCAL"} {
				...
				an_lg $logName "$procName: sending event 60"
				an_cmd sendEvent event=60
				clearErrorSimulations
				chooseWhichErrorToSimulate
			} elseif {$rcmd == "START"} {
				an_cmd setProcessState state=Executing
				an_lg $logName "$procName: sending event 100"
				an_cmd sendEvent event=100
				setupErrorSimulations
			} elseif {$rcmd == "STOP"} {
				an_cmd setProcessState state=Abort
				an_msgsend $currenv [dv_get >name] "cancel id=E noreply"
				an_msgsend $currenv [dv_get >name] "cancel id=C noreply"
				an_msgsend $currenv [dv_get >name] "cancel id=A noreply"
			} elseif {$rcmd == "NEXT"} {
				an_cmd setProcessState state=Ready
			} elseif {$rcmd == "WAIT"} {
				an_cmd setProcessState state=Wait
				an_msgsend $currenv [dv_get >name] "cancel id=E noreply"
				an_msgsend $currenv [dv_get >name] "cancel id=C noreply"
				an_msgsend $currenv [dv_get >name] "cancel id=A noreply"
			} elseif {$rcmd == "CONTINUE"} {
				an_cmd setProcessState state=Executing
				an_msgsend $currenv [dv_get >name] "send msg=do=processingComplete id=C seconds=10 noreply"
			}	
		}
	}
...
global ErrorSimulations
array set ErrorSimulations {
	# 00 Normal but don't add into here
	09 PreStartQueryFail
	19 EventReportSetupFail
	29 CriticalAlarms
	39 ToolPauses
	49 ToolPausesAndResumes
	59 ToolAborts
	69 ProcessNeverCompletes
}
proc chooseWhichErrorToSimulate {} {
	set procName [lindex [info level 0] 0]
	set logName "[dv_get >name]_log"
	an_lg $logName "$procName: starting"
	global whichError ErrorSimulations
	set whichError "Normal"
	an_cmd store file=out.txt 

	foreach CPNAMEPtr [dv_getgbs >sys>msg>CPNAME>*] {
		an_lg $logName "$procName: CPNAME is [dv_get -root $CPNAMEPtr]"
		if {[string range [dv_get -root $CPNAMEPtr] 0 4] == "LOTID"} {
			set LOTID [dv_get >sys>msg>CPVAL>[dv_getname -root $CPNAMEPtr]]
			set lastTwoDigits [string range $LOTID [expr [string length $LOTID] - 2] end]
			an_lg $logName "$procName: LOTID is $LOTID lastTwoDigits is $lastTwoDigits"
			if {[info exists ErrorSimulations($lastTwoDigits)]} {
				set whichError $ErrorSimulations($lastTwoDigits)
				an_lg $logName "$procName: whichError is $whichError"
			}
		}
	}
	an_lg $logName "$procName: ending with whichError $whichError"
}
proc clearErrorSimulations {} {
	an_cmd setProcessState state=Ready
		an_cmd setAck streamFunction=S2F37 good
}
proc setupErrorSimulations {} {
	set procName [lindex [info level 0] 0]
	set logName "[dv_get >name]_log"
	an_lg $logName "$procName: starting"
	global whichError
	an_lg $logName "$procName: Setting up for $whichError"
	if {$whichError == "Normal"} {
		an_msgsend 1 [dv_get >name] "send msg=do=sendSomeEvents id=E seconds=5 noreply"
		an_msgsend 2 [dv_get >name] "send msg=do=sendSomeAlarms id=A seconds=3 noreply"
		an_msgsend 3 [dv_get >name] "send msg=do=processingComplete id=C seconds=20 noreply"
	} elseif {$whichError == "PreStartQueryFail"} {
		# This has to be implemented before
		an_cmd setProcessState state=Off
	} elseif {$whichError == "EventReportSetupFail"} {
		an_cmd setAck streamFunction=S2F37 bad
	} elseif {$whichError == "CriticalAlarms"} {
		an_lg $logName "$procName: sending critical alarm 1 in 5 seconds"
		an_msgsend 4 [dv_get >name] "send msg=do=\"sendAlarm ALID=1 ALCD=10 ALTX=\\\"Shutter moving error\\\"\" id=A2 seconds=5 noreply"
	} elseif {$whichError == "ToolPauses"} {
		an_msgsend 4 [dv_get >name] "send msg=do=\"setProcessState state=Wait\" id=F seconds=5 noreply"
	} elseif {$whichError == "ToolPausesAndResumes"} {
		an_msgsend 4 [dv_get >name] "send msg=do=\"setProcessState state=Wait\" id=F seconds=5 noreply"
		an_msgsend 5 [dv_get >name] "send msg=do=\"setProcessState state=Executing\" id=D seconds=10 noreply"
		an_msgsend 6 [dv_get >name] "send msg=do=processingComplete id=C seconds=20 noreply"
	} elseif {$whichError == "ToolAborts"} {
		an_msgsend 4 [dv_get >name] "send msg=do=\"setProcessState state=Abort\" id=F seconds=5 noreply"
	} elseif {$whichError == "ProcessNeverCompletes"} {
		an_msgsend 1 [dv_get >name] "send msg=do=sendSomeEvents id=E seconds=5 noreply"
		an_msgsend 2 [dv_get >name] "send msg=do=sendSomeAlarms id=A seconds=3 noreply"
	}
...
an_proc sendSomeAlarms {sendSomeAlarms} {doc} {{0 Success}} {
	set procName [lindex [info level 0] 0]
	set logName "[dv_get >name]_log"
	an_lg $logName "$procName: starting"

	an_lg $logName "$procName: sending alarm 1301"
	an_cmd sendAlarm ALID=1301 ALCD=10 ALTX="Bad vacuum"

	an_lg $logName "$procName: sending alarm 1303 in 3 seconds"
	an_msgsend $currenv [dv_get >name] "send msg=do=\"sendAlarm ALID=1303 ALCD=10 ALTX=\\\"Heat water\\\"\" id=A2 seconds=3 noreply"

	an_lg $logName "$procName: sending alarm 267 in 6 seconds"
	an_msgsend $currenv [dv_get >name] "send msg=do=\"sendAlarm ALID=267 ALCD=80 ALTX=\\\"Gas Control Error\\\"\" id=A3 seconds=6 noreply"
